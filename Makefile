...
all: setting_nfs deploy_k8s pause configuring_access_to_k8s tunnel configure_deploy
...
configure_deploy:
	...
	helm repo update
	helm upgrade --install nfs-server nfs-ganesha-server-and-external-provisioner/nfs-server-provisioner --create-namespace -n $(ns)

deploy: deploy_monitoring deploy_app deploy_jenkins
...
deploy_jenkins:
	helm upgrade --install jenkins-sa ./src/deploy/jenkins/jenkins-sa -n $(ns)
